...
class VisionService:
    ...
    def _get_box_color(waste_category: str) -> tuple[int, int, int]:
        category_colors = {
            ...
            "veículo": (255, 255, 255),
            "não resíduo": (100, 100, 100),
        }
        return category_colors.get(waste_category, (0, 0, 255))

    def _estimate_waste_volume(
        self,
        ...
        if occupied_ratio < 0.02:
            severity, estimated_volume_label = "baixo", "pequeno"
        elif occupied_ratio < 0.08:
            severity, estimated_volume_label = "médio", "moderado"
        elif occupied_ratio < 0.18:
            severity, estimated_volume_label = "alto", "grande"
        else:
    ...
    def _draw_detections(
        ...
        saved = cv2.imwrite(str(output_image_path), annotated_image)
        if not saved:
    ...
    def predict_image(self, image_path: str) -> Dict[str, Any]:
        try:
            if self.model is None:
                return {
                    "success": False,
                    "message": "Modelo YOLO não carregado.",
                    "detections": [],
                    "summary": {
                        "total_detections": 0,
                ...
                for index, box in enumerate(result.boxes):
                    ...
                    detections.append({
                        ...
                        "normalized_area": round(
                            self._normalize_area(area, float(image_width * image_height)),
                            6,
                        ),
                    })
