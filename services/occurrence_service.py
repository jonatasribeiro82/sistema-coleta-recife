...
class OccurrenceService:
    ...
    @staticmethod
    def _define_priority(location_analysis: Optional[Dict[str, Any]], vision_analysis: Optional[Dict[str, Any]]) -> str:
        ...
        if severity == "crítico": return "crítica"
        if severity == "alto": return "alta"
        if severity == "médio": return "média"
        if estimated_items_count > 0: return "baixa"
        return "monitoramento"    

    @staticmethod
    def _define_status(vision_analysis: Optional[Dict[str, Any]]) -> str:
        if not vision_analysis: return "recebida"
        if not vision_analysis.get("success"): return "erro_processamento"
    ...
    def build_occurrence_payload(
        self, image_path: Optional[str] = None, camera_id: Optional[str] = None,
        source_type: str = "manual_upload", reported_by: str = "sistema",
        location_analysis: Optional[Dict[str, Any]] = None, vision_analysis: Optional[Dict[str, Any]] = None,
        ...
        return {
            ...
            "source": {"source_type": source_type, "image_path": image_path, "camera_id": camera_id, "reported_by": reported_by},
            "status": self._define_status(vision_analysis),
            "priority": self._define_priority(location_analysis, vision_analysis),
    ...
    def save_occurrence(self, occurrence_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"success": True, "occurrence_id": occurrence_id, "file_path": str(output_file), "message": "Ocorrência salva com sucesso."}